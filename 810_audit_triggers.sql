CREATE OR REPLACE FUNCTION trigger_set_audit_update_fields()
RETURNS TRIGGER AS $$
  BEGIN
    NEW.updated_at = now();
...
CREATE OR REPLACE FUNCTION trigger_set_audit_create_fields()
RETURNS TRIGGER AS $$
  BEGIN
    NEW.created_at = now();
...
CREATE TRIGGER audit_fields_on_create BEFORE INSERT ON app_public.users
  FOR EACH ROW EXECUTE PROCEDURE trigger_set_audit_create_fields();

CREATE TRIGGER audit_fields_on_update BEFORE UPDATE ON app_public.users
  FOR EACH ROW EXECUTE PROCEDURE trigger_set_audit_update_fields();
